...
export default function Blog() {
  ...
  // const [rateBlog] = useRateBlogMutation();

  // if ();

  // const updateRateBlogHandler = async (action: Action) => {
  //   const res = await rateBlog({ id, action });
  //   console.log(res);
  //   refetchBlogStats();
  //   console.log(blogStats);
  // };

  if (requestedBlogIsLoading) return <Spinner />;
  return (
    <div className="px-6 sm:px-20">
      ...
      {/* <div className="mt-8 flex">
        <button
          className={`font-satoshi-bold flex items-center justify-center gap-x-3 rounded-bl-xl rounded-tl-xl border-2 border-white border-opacity-50 ${likeButtonActive ? "bg-green-500" : "bg-purple-500"} px-3 py-2 text-3xl transition-all hover:border-double hover:border-opacity-100 hover:${likeButtonActive ? "bg-green-600" : "bg-purple-600"}`}
          onClick={() => {
            updateRateBlogHandler(likeButtonActive ? "unlike" : "like");
        ...
        <button
          className={`font-satoshi-bold flex items-center justify-center gap-x-3 rounded-br-xl rounded-tr-xl border-2 border-white border-opacity-50 ${dislikeButtonActive ? "bg-green-500" : "bg-purple-500"} px-3 py-2 text-3xl transition-all hover:border-double hover:border-opacity-100 hover:${dislikeButtonActive ? "bg-green-600" : "bg-purple-600"}`}
          onClick={() => {
            updateRateBlogHandler(
              dislikeButtonActive ? "undislike" : "dislike"
            );
            if (likeButtonActive) setLikeButtonActive(false);
            setDislikeButtonActive(!dislikeButtonActive);
          }}
        >
