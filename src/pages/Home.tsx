...
const noThumbnail = (
  <div className="flex h-[5.5rem] items-center justify-center rounded-tl-2xl rounded-tr-2xl bg-white">
    <p className="font-satoshi-light-italic text-xl text-black">No Thumbnail</p>
  </div>
);
    ...
    return (
      <Link to={`blog/${blogId}`} key={blogId}>
        <section className="h-60 w-80 overflow-hidden rounded-2xl border-2 border-[#393E46] bg-slate-900 transition-all duration-300 hover:border-double hover:border-white hover:shadow-2xl">
          {blog.thumbnail ? (
            <Color src={blog.thumbnail} format="hex">
              {({ data: colorData, loading, error }) => {
                console.log(error);
                if (loading || error) return <div className="h-[5.5rem]"></div>;
                return (
                  <div
                    className={`flex justify-center rounded-tl-2xl rounded-tr-2xl bg-[${colorData}] bg-opacity-40 hue-rotate-180`}
                  >
                    <img
                      src={blog.thumbnail}
                      className="my-3 h-16 hue-rotate-180"
                    />
                  </div>
                );
              }}
